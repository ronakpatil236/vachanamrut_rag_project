...
def run_single_eval(test_item: TestItem) -> Dict[str, Any]:
  try:
    # 1. Transform Query (Step 5) to expand terms into Vachanamrut vocabulary
    transformed_query = transform_query(test_item.question)

    # 2. Retrieval Evaluation (Retrieving using the TRANSFORMED query)
    retrieved_chunks = get_hybrid_chunks(transformed_query, k=4)
    retrieval_eval = evaluate_retrieval(test_item, retrieved_chunks)

    # 3. Answer Generation & Evaluation
    # Pass user_query/question for answer generation context, but transformed_query for retrieval
    try:
      generated_ans = answer_question(
          user_query=test_item.question,
          search_query=transformed_query,
          retriever_type="hybrid",
          store_type="structural",
      )
    except TypeError:
      # Fallback if step4_retriever.py hasn't updated its function signature yet
      generated_ans = answer_question(
          test_item.question, retriever_type="hybrid", store_type="structural"
      )
    ...
    return {
        ...
        "keywords_found": (
            f"{retrieval_eval.keywords_found}/{retrieval_eval.total_keywords}"
        ),
    ...
    return {
        "question": test_item.question,
        "transformed_query": f"Error during transform: {str(e)}",
        "category": test_item.category,
        "source_file": test_item.source_file,
        "source_found": False,
        "mrr": 0.0,
...
def run_all_evaluations(
  ...
  print(
      f"\n🚀 Running Transformed Hybrid evaluation for {total_tests} tests with"
      f" {max_workers} worker thread(s)...\n"
  )
...
def main():
  parser = argparse.ArgumentParser(
      description="Run Vachanamrut RAG Benchmark Suite (Hybrid + Query Transform)"
  )
  parser.add_argument(
      "--test_data",
      type=str,
  ...
  parser.add_argument(
      "--output_csv",
      type=str,
      default=str(RESULTS_DIR / "eval_results_transformed.csv"),
  ...
  print("\n" + "=" * 50)
  print(f"✅ Transformed Hybrid Evaluation Complete!")
